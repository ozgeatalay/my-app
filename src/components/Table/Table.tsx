...
export const MagicTable = () => {
  ...
  return (
    <div>
      <Button buttonWidth="50px" clickHandler={() => addButtonHandler()} text="Add"/>
       <Table>
      <thead>
      <tr>
...

        {
          ...
          <tr key={data.id}>
         <td>
             <Input onChange={(event) => changeHandler(event, data.id)} type="text" width="180px" name="label" defaultValue={tableData[index].label} />
          </td>
           <td>
             <Input onChange={(event) => changeHandler(event, data.id)} type="number" width="100px" name="vision" defaultValue={tableData[index].vision}  />
          </td>
         <td>
            <Input onChange={(event) => changeHandler(event, data.id)}   type="number" width="100px" name="ability" defaultValue={tableData[index].ability} />
          </td>
         <td>
            <Button  buttonWidth="100px" clickHandler={() => handleDeleteButton(data.id)} text="Delete"/>
         </td>
          </tr>
          )
        }
